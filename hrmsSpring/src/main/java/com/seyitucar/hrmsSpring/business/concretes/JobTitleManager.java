...
public class JobTitleManager implements JobTitleService {
	...
	public Result update(JobTitle jobTitle) {
		JobTitle updJobTitle = this.jobTitleDao.getById(jobTitle.getId());
		
		updJobTitle.setTitleName(jobTitle.getTitleName());
		this.jobTitleDao.save(updJobTitle);
		return new SuccessResult("Pozisyon adı güncellendi");
	}

	@Override
	public Result delete(int id) {
		this.jobTitleDao.deleteById(id);
		return new SuccessResult("Pozisyon bilgisi silindi");
	}

	@Override
	public DataResult<JobTitle> getById(int id) {
		return new SuccessDataResult<JobTitle>(this.jobTitleDao.findById(id).orElse(null));
	}
